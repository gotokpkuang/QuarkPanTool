...
class QuarkPanFileManager:
    ...
    def get_fids(self, file_paths: list) -> list:
        """
        根据文件路径获取文件ID
        
        Args:
            file_paths: 文件路径列表，例如 ["/folder1/file1.txt", "/folder2"]
            
        Returns:
            list: 包含fid和file_name的字典列表
        """
        fids = []
        paths_to_query = file_paths.copy()
        
        while paths_to_query:
            api = "https://drive-pc.quark.cn/1/clouddrive/file/info/path_list"
            params = {
                'pr': 'ucpro',
                'fr': 'pc',
                'uc_param_str': '',
            ...
            # 每次最多查询50个路径
            batch = paths_to_query[:50]
            paths_to_query = paths_to_query[50:]
            ...
            try:
                import requests
                response = requests.post(api, json=payload, params=params, headers=self.headers, timeout=60)
                json_data = response.json()
                
                if json_data.get('code') == 0 and json_data.get('data'):
                    fids.extend(json_data['data'])
                else:
                    custom_print(f"获取目录ID失败: {json_data.get('message', '未知错误')}", error_msg=True)
                    break
            except Exception as e:
                custom_print(f"获取目录ID时出错: {str(e)}", error_msg=True)
                break
    ...
    def mkdir_recursive(self, dir_path: str) -> dict:
        ...
        payload = {
            'pdir_fid': '0',
            'file_name': '',
            'dir_path': dir_path,  # 使用dir_path参数支持递归创建
            'dir_init_lock': False,
        }
        ...
        try:
            ...
            if json_data.get('code') == 0:
                custom_print(f'目录创建成功: {dir_path}')
                return json_data
            elif json_data.get('code') == 23008:
                # 文件夹已存在，不算错误
                custom_print(f'目录已存在: {dir_path}')
                return json_data
            else:
                custom_print(f"目录创建失败: {json_data.get('message', '未知错误')}", error_msg=True)
                return json_data
        except Exception as e:
            custom_print(f"创建目录时出错: {str(e)}", error_msg=True)
            return {'code': -1, 'message': str(e)}

    async def list_files_recursive(self, pdir_fid: str = '0', base_path: str = '') -> list:
        """
        递归列出目录下的所有文件（用于自己网盘的文件）
        ...
        Returns:
            list: 文件信息列表，每个文件包含 name, size, is_dir, relative_path 等信息
        """
        all_files = []
        
        # 获取当前目录的文件列表
        json_data = await self.get_sorted_file_list(pdir_fid, page='1', size='100', fetch_total='1')
        
        if json_data.get('code') != 0 or not json_data.get('data'):
            return all_files
        
        file_list = json_data['data'].get('list', [])
        ...
        for item in file_list:
            ...
            # 打印文件信息供调试
            custom_print(f"文件信息: {file_info}")
            
            all_files.append(file_info)
            
            # 如果是目录，递归获取子目录内容
            if item.get('dir'):
                sub_files = await self.list_files_recursive(item['fid'], relative_path)
